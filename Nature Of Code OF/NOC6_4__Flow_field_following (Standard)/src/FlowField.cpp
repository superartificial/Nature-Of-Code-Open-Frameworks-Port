...
void FlowField::populate() {
     for(int rowPos=0; rowPos< rows ; rowPos++)
    {
        for(int colPos=0; colPos< cols ; colPos++)
        {
            ...
            float angle = ofNoise(colPos * 0.01,rowPos * 0.01,ofGetFrameNum() * 0.001) * 360;
...
void FlowField::display()
{
    ...
    for(int rowPos=0; rowPos< rows ; rowPos++)
    {
        for(int colPos=0; colPos< cols ; colPos++)
        {
            ...
            // if(colPos>(cols/2)) angleToRotate -= 180 ;
            //  angleToRotate -= 180 ;
            ofSetColor(255,0,0,255);
            ofLine(0, 0, flowAtPos.x * resolution,flowAtPos.y * resolution);
